...
export default function ParrainagePage() {
  const t = useTranslations("medecin.parrainage");
    ...
    <div className="space-y-6">
      ...
      <div className="ds-card p-6 shadow-sm space-y-4">
        ...
        {searchQuery.trim().length >= 2 && !searching && searchResults.length === 0 && (
          <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-2">{t("noDoctorsFound")}</p>
        )}
      </div>
      ...
      <div className="ds-card">
        ...
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <p className="text-foreground font-medium mb-1">{t("emptyStateTitle")}</p>
            <p className="text-sm text-gray-400 dark:text-gray-500">{t("emptyStateSubtitle")}</p>
          </div>
        ) : (
              ...
              <div key={r.id} className="p-4 flex items-center justify-between hover:bg-secondary transition-colors">
                <div>
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {r.referredEmail} &middot; {t("registeredOn")}{" "}
                    {format(new Date(r.createdAt), "d MMM yyyy", { locale: fr })}
                    {r.validatedAt && (
                      <> &middot; {t("validatedOn")} {format(new Date(r.validatedAt), "d MMM yyyy", { locale: fr })}</>
                    )}
                  </div>
                </div>
                <span
