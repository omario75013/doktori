...
export default async function VerificationOrdonnancePage({
  ...
  if (!token) {
    return (
      <main className="min-h-screen bg-[#F0FDFA] flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-2xl border border-[#E6F4F1] shadow-sm p-8 text-center">
          <ShieldCheck className="w-12 h-12 text-[#0891B2] mx-auto mb-4" />
          <h1 className="text-xl font-bold text-[#134E4A] mb-2">
  ...
  return (
    <main className="min-h-screen bg-[#F0FDFA] flex items-center justify-center px-4 py-12">
          ...
          <div className="bg-white rounded-2xl border-2 border-green-400 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-semibold text-[#5E7574] uppercase tracking-wide mb-1">Médecin</p>
                  <p className="text-sm font-bold text-[#134E4A]">{result.doctorName}</p>
                  {specialtyLabel && (
                    <p className="text-xs text-[#0891B2]">{specialtyLabel}</p>
                  )}
                </div>
                <div>
                  <p className="text-xs font-semibold text-[#5E7574] uppercase tracking-wide mb-1">Patient</p>
                  <p className="text-sm font-bold text-[#134E4A]">{result.patientName}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-[#5E7574] uppercase tracking-wide mb-1">Date</p>
                  <p className="text-sm font-bold text-[#134E4A]">{formattedDate}</p>
                </div>
              </div>

              {result.content && (
                <div className="mt-4 pt-4 border-t border-[#E6F4F1]">
                  <p className="text-xs font-semibold text-[#5E7574] uppercase tracking-wide mb-2">Contenu</p>
                  <div className="bg-[#F0FDFA] rounded-xl p-4 text-sm text-[#134E4A] whitespace-pre-wrap leading-relaxed max-h-48 overflow-y-auto">
                    {result.content}
                  </div>
                </div>
              )}
              ...
              <div className="mt-4 pt-4 border-t border-[#E6F4F1] text-center">
                <p className="text-xs text-[#5E7574]">
                  ...
                  <span className="font-bold text-[#0891B2]">Doktori.tn</span>
                </p>
              </div>
            </div>
          </div>
