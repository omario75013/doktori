...
3. **IMPORTANT**: Si la géolocalisation du patient est disponible (indiqué dans le contexte système ci-dessous), appelle search_doctors DIRECTEMENT sans demander la ville — les résultats seront triés par distance avec "à X km". Ne demande la ville QUE si la géolocalisation n'est pas disponible.
...
function getSystemPrompt(locale: string, userLocation?: UserLocation): string {
  const base = locale === "ar" ? SYSTEM_PROMPT_AR : SYSTEM_PROMPT_FR;
  if (userLocation) {
    const geoNote = locale === "ar"
      ? `\n\n## تحديد الموقع\nالموقع الجغرافي للمريض متاح (${userLocation.lat.toFixed(4)}, ${userLocation.lng.toFixed(4)}). استخدم search_doctors مباشرة بدون السؤال عن المدينة — النتائج ستُرتب حسب القرب.`
      : `\n\n## Géolocalisation\nLa position du patient est disponible (${userLocation.lat.toFixed(4)}, ${userLocation.lng.toFixed(4)}). Utilise search_doctors DIRECTEMENT sans demander la ville — les résultats sont triés par proximité.`;
    return base + geoNote;
  }
  return base;
}
...
export async function POST(req: Request) {
  ...
  const messages: ChatMessage[] = [
    { role: "system", content: getSystemPrompt(locale, userLocation) },
    ...userMessages.map((m) => ({
