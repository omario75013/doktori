...
import { db, prescriptions, appointments, doctors } from "@doktori/db";
import { and, eq, inArray, ne } from "drizzle-orm";
import { requireAuth } from "@/lib/require-auth";

/**
 * PATCH /api/prescriptions/[id]/share
 * Body: { doctorIds: string[] }
 *
 * Updates the shared_with_doctor_ids list on a prescription.
 * Only the owning doctor can share their own prescription.
...
export async function PATCH(
  ...
  const [existing] = await db
    ...
    .from(prescriptions)
    .where(
      and(eq(prescriptions.id, id), eq(prescriptions.doctorId, user.id)),
    )
    .limit(1);
