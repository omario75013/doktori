...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let [patientRecord] = await db.select().from(patients).where(eq(patients.phone, phone)).limit(1);
    if (!patientRecord) {
      [patientRecord] = await db.insert(patients).values({ name: patientName, phone }).returning();
    }
    ...
    console.error("[POST /api/home-visit/request]", e);
